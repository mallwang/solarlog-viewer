...
Added sections:
  - Technical Standards (frontend, backend, package manager, testing, linting, formatting)
  - Documentation Standards (README, user guides, JSDoc, file-level descriptions)
  - Development Workflow (spec → plan → tests → code)
Templates checked:
...
produced today. No server-side preprocessing, transformation pipelines, or data
migration scripts may be introduced.
...
database, no build-time pipeline beyond what the SolarLog device pushes. This
constraint MUST be preserved. The modernized site MUST remain deployable to any
plain web host or static file server (Apache, nginx, GitHub Pages, S3, etc.)
with no runtime dependencies.

All data parsing, aggregation, and chart rendering MUST run client-side in the
browser.
...
- Adding a server-side application layer.
...
- Reprocessing or re-aggregating historical minute data (display only).
...
Per Principle III, there is no backend. All logic runs client-side.
