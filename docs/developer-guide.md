...
every other field). The Statistics page's daily-granularity "best of" picks and streaks
(`web/js/data/statistics.js`'s `excludeBackfilledDays`) need to know which dates those are, so
they don't let a reconstructed value win a spurious record. That list lives in
