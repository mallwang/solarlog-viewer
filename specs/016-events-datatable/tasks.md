...
- [x] T001 Confirm on branch `016-events-datatable` with a clean working tree (`git status`) before
...
- [x] T002 [P] Extend `parseBaseVars()` in `web/js/data/plant.js` to also return `statusCodes:
...
- [x] T003 [P] Add cases to `web/js/data/plant.test.js` for the new `statusCodes`/`errorCodes`
...
- [x] T004 [US-shared] Create `web/js/data/events.js` implementing, per contracts/events.md:
...
- [x] T005 [P] [US-shared] Create `web/js/data/events.test.js` covering (per quickstart.md):
...
- [x] T006 [US1] Add the `#/events` route to `web/js/router.js`: `parseRoute` recognizes
...
- [x] T007 [US1] Add new `events` i18n namespace + `nav.eventsView` key to `web/i18n/de.json`
      ...
      i18n contract.
- [x] T008 [P] [US1] Add the matching English key set to `web/i18n/en.json` (same structure as
      T007, English copy) — key sets MUST match exactly between the two files.
- [x] T009 [US1] Create `web/js/views/events-view.js` with `export async function render(container,
...
- [x] T010 [US1] Register the new view in `web/js/main.js`: add `events: () =>
...
- [x] T011 [P] [US1] Add `.events-page`/`.events-table`/status-pill/ongoing-badge styles to
...
- [x] T012 [P] [US1] Create `web/js/views/events-view.test.js` with initial coverage for any pure
...
- [x] T013 [US1] Create `tests/e2e/events-view.spec.js` with the first two Playwright scenarios
...
- [x] T014 [P] [US2] Add `export function filterEvents(events, filters)` and `export function
...
- [x] T015 [P] [US2] Add cases to `web/js/views/events-view.test.js` for `filterEvents` (each
...
- [x] T016 [US2] Wire the filter bar into `events-view.js`'s `render()`: four labeled dropdowns
...
- [x] T017 [US2] Add active-filter pill chips directly under the filter bar (one per active
...
- [x] T018 [US2] Add the empty-state branch: when the active filters match zero events, replace
...
- [x] T019 [US2] Extend `tests/e2e/events-view.spec.js` with contracts/events.md scenarios 3 and 5:
...
- [x] T020 [P] [US3] Add `export function sortEvents(events, sort)` to `web/js/views/events-
...
- [x] T021 [P] [US3] Add cases to `web/js/views/events-view.test.js` for `sortEvents`: each column
...
- [x] T022 [US3] Wire sortable column headers (Von–Bis, WR, Dauer) into `events-view.js`'s table
...
- [x] T023 [US3] Extend `tests/e2e/events-view.spec.js` with contracts/events.md scenario 4:
...
- [x] T024 [P] Add the mobile-viewport Playwright scenario (contracts/events.md scenario 6) to
...
- [x] T025 [P] Update `README.md` and `README.de.md` to document the new Ereignisse page (plan.md
      Documentation Standards, quickstart.md's constitution checklist).
- [x] T026 [P] Update `docs/user-guide.md` and `docs/user-guide.de.md` with how to reach the
      Ereignisse page and its filter/sort behavior.
- [x] T027 Run `npm run lint` and `npm run format:check`; fix any errors (quickstart.md's
      constitution/documentation checklist).
- [x] T028 Run the full quickstart.md validation: all `node --test` files, the full
