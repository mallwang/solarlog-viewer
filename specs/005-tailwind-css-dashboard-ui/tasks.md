...
- [x] T001 Add `tailwindcss` (v4) as an npm devDependency and `apexcharts` build source in
...
- [x] T002 Run `npm install` to lock the new devDependency in `package-lock.json`
- [x] T003 [P] Vendor ApexCharts ESM/UMD build into `web/vendor/apexcharts/` (replacing
      `web/vendor/chart.js/` per plan.md Project Structure), matching the existing Chart.js
      vendoring pattern (research.md §4)
- [x] T004 [P] Create `web/css/tailwind.css` with `@import "tailwindcss";` plus a `@theme` block
...
- [x] T005 Run `npm run build:css` to produce the initial `web/css/tailwind.generated.css` and
      commit it as a build artifact (plan.md Project Structure; FR-012)
- [x] T006 Add `<link rel="stylesheet" href="css/tailwind.generated.css">` to `web/index.html`
...
- [x] T007 Rewrite `web/js/charts/chart-factory.js` internals to build ApexCharts option objects
...
- [x] T008 Keep the `typeof window !== 'undefined'` guard in `web/js/charts/chart-factory.js` so it
      does not throw when imported in the DOM-less `node:test` environment (contracts/
      chart-factory.md Compatibility note)
- [x] T009 [P] Extend `renderNav()`/`NAV_ITEMS` in `web/js/main.js` to render the full markup
...
- [x] T010 [P] Add nav toggle open/close vanilla JS in `web/js/main.js`: click toggles
...
- [x] T011 [P] Add new i18n keys (nav `aria-label`, toggle accessible label, empty-state text,
      "not producing" status text) to `web/i18n/de.json` and `web/i18n/en.json`
- [x] T012 [P] Add Tailwind utility classes for `.app-nav`, `.app-nav__toggle`, `.app-nav__list`
...
- [x] T013 Update each of `web/js/views/dashboard.js`, `day-view.js`, `month-view.js`,
...
- [x] T014 Remove `web/vendor/chart.js/` and the `chart.js` devDependency from `package.json` now
...
- [x] T015 [P] [US1] Playwright spec `tests/e2e/dashboard-consistency.spec.js`: navigate to all six
...
- [x] T016 [P] [US1] Playwright spec `tests/e2e/dashboard-dark-mode.spec.js`: use
...
- [x] T017 [P] [US1] Playwright spec `tests/e2e/dashboard-charts.spec.js`: open day, month, year,
...
- [x] T018 [P] [US1] Playwright spec `tests/e2e/dashboard-empty-state.spec.js`: navigate to a
...
- [x] T019 [P] [US1] Playwright spec `tests/e2e/dashboard-status.spec.js`: assert the
...
- [x] T020 [US1] Migrate `web/js/views/dashboard.js`'s summary-stat cards (current power,
...
- [x] T021 [US1] Implement the `SummaryStat.status` non-color indicator (icon + text label
      alongside color) in `web/js/views/dashboard.js`, driven by `producing`/`idle`/`unavailable`
      (FR-010)
- [x] T022 [P] [US1] Migrate `web/js/views/day-view.js` chart container and layout to Tailwind
      utility classes (chart-mount `<div>` styling only — data/logic unchanged)
- [x] T023 [P] [US1] Migrate `web/js/views/month-view.js` chart container and summary elements to
      Tailwind utility classes
- [x] T024 [P] [US1] Migrate `web/js/views/year-view.js` chart container and summary elements to
      Tailwind utility classes
- [x] T025 [P] [US1] Migrate `web/js/views/total-view.js` lifetime-total summary elements to
      Tailwind utility classes (data-model.md `SummaryStat`)
- [x] T026 [P] [US1] Migrate `web/js/views/compare-view.js` chart container and layout to Tailwind
      utility classes
- [x] T027 [US1] Implement the shared FR-009 empty/placeholder-state component (used by
      ...
      module
- [x] T028 [US1] Trim now-redundant rules from `web/css/app.css` as each view's markup migrates to
      Tailwind utility classes, keeping only rules with no Tailwind equivalent
- [x] T029 [US1] Verify `web/css/tokens.css` light/dark `--color-*`/`--chart-color-*` values are
...
- [x] T030 [P] [US2] Playwright spec `tests/e2e/dashboard-nav.spec.js`: from each of the six
      ...
      quickstart.md §2
- [x] T031 [P] [US2] Extend `tests/e2e/dashboard-nav.spec.js` (or add a case) asserting that
...
- [x] T032 [US2] Verify/finish `NAV_ITEMS` in `web/js/main.js` covers all 6 routes from
      `router.js` in fixed order (data-model.md `NavigationMenu.items` validation rule) — add any
      missing i18n `labelKey`s
- [x] T033 [US2] Ensure active-state (`aria-current="page"`) recomputation happens synchronously
...
- [x] T034 [US2] Style the persistent (`md:` and above) and collapsible (below `md:`) nav layouts
...
- [x] T035 [P] [US3] Playwright spec `tests/e2e/dashboard-responsive.spec.js`: set viewport widths
...
- [x] T036 [US3] Extend `tests/e2e/dashboard-responsive.spec.js` with a case for the `compare`
...
- [x] T037 [US3] Audit and fix any remaining fixed-width/overflow issues in
...
- [x] T038 [P] [US3] Audit and fix responsive Tailwind classes across
...
- [x] T039 [US3] Confirm ApexCharts instances in `web/js/charts/chart-factory.js` resize with
...
- [x] T040 [P] Update `README.md` and `README.de.md` to describe the new Tailwind-based visual
      design and responsive navigation (plan.md Constitution Check, Documentation Standards)
- [x] T041 [P] Update `docs/user-guide.md` and `docs/user-guide.de.md` to reflect the new nav
      interaction (hamburger on mobile, persistent on desktop) and ApexCharts tooltip behavior
- [x] T042 Run `npm run lint` and `npm run format:check`; fix any violations across changed files
      (constitution Technical Standards)
- [x] T043 Run `npm run test:scripts` (node:test unit suites, including router/aggregates) and
      confirm all pass unchanged (SC-005)
- [x] T044 Run `npm test` (full Playwright suite): confirm the pre-existing
...
- [x] T045 Execute quickstart.md's full validation walkthrough (§1–7) manually via `npm start` in
