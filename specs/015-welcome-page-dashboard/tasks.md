...
- [ ] T001 Create `web/img/plant/` directory (with a `.gitkeep` or a first operator-provided photo if available) so the path referenced by `PLANT_PHOTOS` resolves; no code in this directory (plan.md Structure Decision).
- [ ] T002 [P] Add `PLANT_PHOTOS: string[]` (default `[]`) to `web/js/config.js`, documented per the `module-contracts.md` `config.js` contract (JSDoc: filenames under `web/img/plant/`, carousel display order, empty = FR-008 placeholder).
...
- [ ] T009 [US1] In `web/js/views/welcome-view.js`, mount `plantDetailsMarkup(ctx.plant)` into the primary region below the carousel mount point, wrapped in its own try/catch so a rendering failure here doesn't blank the carousel or chart regions (FR-013, SC-004).
- [ ] T010 [US1] In `web/js/views/welcome-view.js`, fetch today's trace the same way `day-view.js` does (`fetchText` + `sourceDirForDate`/`DATA_DIR`/`min_day.js` for today, `parseMinFile`) and call `renderChart(mount, 'day-total', trace, undefined)` into the secondary region when readings exist; render `emptyStateBody('welcome.chartUnavailable')` when the fetch fails or `trace.readings.length === 0` (FR-017, data-model.md "Failure mode"), wrapped in its own try/catch (SC-004).
- [ ] T011 [US1] In `web/js/views/welcome-view.js`, apply the FR-004/FR-005 responsive grid: `grid-cols-1` (stacked: carousel → plant details → chart) below `lg`, `lg:grid-cols-3` with the primary region at `lg:col-span-2` and secondary region at `lg:col-span-1` at `lg` and above, using Tailwind utilities per the existing `dashboard.js`/detail-view grid pattern (no new CSS needed unless a gap emerges).
- [ ] T012 [US1] Give the welcome page's heading/section a title sourced from `welcome.title` (or the plant's own title when set) so the view has the same `h2.view-title` structural element other views have, for layout/test consistency — while not duplicating the header's existing production/yield summary (FR-018).
...
- [ ] T022 [P] Run `npx playwright test --reporter=line` (full suite) — confirms `welcome-page.spec.js` passes and no other existing spec (e.g. `detail-views.spec.js`, `navigation.spec.js`, `dashboard.spec.js`) regressed from the `defaultRoute()` change (quickstart.md "Automated tests").
- [ ] T023 [P] Run `node --test web/js/views/photo-carousel.test.js` (and the full `node --test` unit suite) to confirm the new carousel unit tests pass.
- [ ] T024 Run `npm run lint` and `npm run format:check`; fix any errors (constitution Development Workflow gate 5, mandatory before commit).
- [ ] T025 Manually walk through quickstart.md's "Failure isolation (SC-004)" section (block `data/base_vars.js`, empty `PLANT_PHOTOS`, block/empty today's `min{yymmdd}.js`) confirming the other two regions render normally in each case.
