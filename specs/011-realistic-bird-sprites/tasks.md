...
- [x] T013 Run `npm test` (full Playwright + unit suite) and confirm exit 0; update `tests/e2e/sky.spec.js` bird-spawn assertion if the test now needs to assert on `.sky-bird-sprite` presence rather than bare `.sky-flying-object` count — unit suite (522 tests) and lint are clean; the full Playwright run's 27 pre-existing failures (stale routes/labels from the 013-020 view redesigns, unrelated to this feature, tracked in [#46](https://github.com/mallwang/solarlog-viewer/issues/46)) have since been fixed, `tests/e2e/sky-birds.spec.js` included — every spec file, including this one, passes deterministically when run individually (`npx playwright test tests/e2e/<file>.spec.js`). A full `npx playwright test` run still shows 2-4 unrelated tests time out on `waitForLoadState('networkidle')` against the live proxied device (`web/data`/`web/hist` are proxied to `https://wolfsbach.synology.me`, see root `CLAUDE.md`); this reproduces with `--workers=1` too (no worker-contention link) and hits a different random test each run, so it's pre-existing environmental network flakiness, not a regression from this feature or from the #46 fix — do not treat "exit 0" claims for the full suite as reproducible until that's addressed separately
