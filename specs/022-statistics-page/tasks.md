...
- [x] T001 Create `web/js/views/statistics/` directory with an empty `statistics-view.js` shell
      (route entry point, no rendering logic yet — see plan.md's Project Structure).
- [x] T002 [P] Create empty `web/js/data/statistics.js` with a file-level JSDoc comment describing
      it as the pure computation module per contracts/statistics-module.md (no exports yet).
- [x] T003 [P] Add empty `statistics` key namespaces to `web/i18n/de.json` and `web/i18n/en.json`
...
- [x] T004 Add the `'statistics'` route to `parseRoute`/`formatRoute` in `web/js/router.js`:
...
- [x] T005 [P] Add router test coverage for the new route in `web/js/router.test.js` (or create it
      if it doesn't exist — check first): valid topics, missing topic, invalid topic, and
      `formatRoute` round-trip.
- [x] T006 Add the "Statistik" entry to `NAV_ITEMS` in `web/js/main.js` (after `total`, before
      `events`, per design.md's nav ordering) and register `statistics: () =>
import('./views/statistics/statistics-view.js')` in `viewModules`.
- [x] T007 Implement `statistics-view.js`'s page shell in
...
- [x] T008 [P] Add `web/js/views/statistics/statistics-view.test.js` covering: the shell fetches
...
- [x] T009 [P] Add shared markup helpers used by every topic (stat-tile card, "worst"
...
- [x] T010 [P] Implement `hasEnoughHistory(fullDailyHistory, fullYearlyHistory, topic)` in
      `web/js/data/statistics.js` per data-model.md's "Not-enough-data gating" table (FR-012).
- [x] T011 [P] Add `web/js/data/statistics.test.js` covering `hasEnoughHistory` for each gated
...
- [x] T012 [P] [US1] Unit tests for `bestWorstMonth`/`bestWorstYear` in
...
- [x] T013 [P] [US1] Unit tests for `maxDailyPower`/`maxIstPercent`/`maxDailyCo2`/`maxDailyEuro` in
...
- [x] T014 [P] [US1] Playwright test in `tests/e2e/statistics-view.spec.js` (create file): open
...
- [x] T015 [US1] Implement `bestWorstMonth(fullMonthlyHistory)` and
...
- [x] T016 [US1] Implement `maxDailyPower(fullDailyHistory)`,
...
- [x] T017 [US1] Create `web/js/views/statistics/common-topic.js`: renders the 8-tile grid (4 cols
...
- [x] T018 [US1] Wire `common-topic.js` into `statistics-view.js`'s topic-mounting switch (from
      T007) for `topic === 'common'`.
- [x] T019 [P] [US1] Add `statistics.common.*` and `statistics.commonTiles.*` i18n keys (labels,
...
- [x] T020 [P] [US2] Unit tests for `buildCalendarHeatmap` in `web/js/data/statistics.test.js`:
...
- [x] T021 [P] [US2] Playwright test in `tests/e2e/statistics-view.spec.js`: open
...
- [x] T022 [US2] Implement `buildCalendarHeatmap(fullDailyHistory, year, metric, plant)` in
...
- [x] T023 [US2] Create `web/js/views/statistics/heatmaps-topic.js`: year `<select>` (defaulting to
...
- [x] T024 [US2] Wire `heatmaps-topic.js` into `statistics-view.js` for `topic === 'heatmaps'`,
      including the `hasEnoughHistory(..., 'heatmaps')` gate from T010 rendering the shared
      empty-state (T009) when ungated data is empty.
- [x] T025 [P] [US2] Add `statistics.heatmaps.*` i18n keys (year selector label, three metric
...
- [x] T026 [P] [US3] Unit tests for `computeLongestStreak` in `web/js/data/statistics.test.js`:
...
- [x] T027 [P] [US3] Unit tests for `computeYoyCumulative`, `computeLifetimeCumulative`, and
...
- [x] T028 [P] [US3] Unit tests for the three new `chart-factory.js` modes
...
- [x] T029 [P] [US3] Playwright test in `tests/e2e/statistics-view.spec.js`: open
...
- [x] T030 [US3] Determine and hardcode `STREAK_THRESHOLD_KWH` in `web/js/data/statistics.js` per
...
- [x] T031 [US3] Implement `computeLongestStreak(fullDailyHistory)` in `web/js/data/statistics.js`
...
- [x] T032 [US3] Implement `computeYoyCumulative(fullDailyHistory)`,
...
- [x] T033 [US3] Add the `'yoy-cumulative'` mode (multi-series line, aligned by day-of-year) to
      ...
      `onDataPointClick`.
- [x] T034 [US3] Add the `'lifetime-cumulative'` mode (dual-axis €/CO2 line, point →
      `#/year/YYYY`) to `web/js/charts/chart-factory.js`.
- [x] T035 [US3] Add the `'specific-yield-trend'` mode (per-year bar, bar → `#/year/YYYY`) to
      `web/js/charts/chart-factory.js`.
- [x] T036 [US3] Create `web/js/views/statistics/streaks-topic.js`: oversized streak-length stat
...
- [x] T037 [US3] Create `web/js/views/statistics/trends-topic.js`: three stacked chart blocks via
...
- [x] T038 [US3] Wire `streaks-topic.js` and `trends-topic.js` into `statistics-view.js` for
...
- [x] T039 [P] [US3] Add `statistics.streaks.*` and `statistics.trends.*` i18n keys (including the
...
- [x] T040 [P] [US4] Unit tests for `bestWorstPairs` in `web/js/data/statistics.test.js`: composes
...
- [x] T041 [P] [US4] Playwright test in `tests/e2e/statistics-view.spec.js`: open
...
- [x] T042 [US4] Implement `bestWorstPairs(fullDailyHistory, fullMonthlyHistory,
...
- [x] T043 [US4] Create `web/js/views/statistics/best-worst-topic.js`: label/best/worst
...
- [x] T044 [US4] Wire `best-worst-topic.js` into `statistics-view.js` for `topic === 'best-worst'`
      (ungated per data-model.md — always renders).
- [x] T045 [P] [US4] Add `statistics.bestWorst.*` i18n keys to `web/i18n/de.json` and
      `web/i18n/en.json`.
...
- [x] T046 [P] Verify/adjust responsive CSS for the topic nav (sticky vertical → horizontal
...
- [x] T047 [P] Playwright responsive test in `tests/e2e/statistics-view.spec.js`: resize to 320px
      and 2560px, assert no horizontal scroll and the topic nav collapses to a button row on
      mobile (quickstart.md step 6).
- [x] T048 [P] Playwright edge-case test in `tests/e2e/statistics-view.spec.js`: with a
...
- [x] T049 [P] Update `docs/user-guide.md` and `docs/user-guide.de.md` with a Statistics page
      section (topics, what each shows) per Documentation Standards.
- [x] T050 [P] Update `README.md` and `README.de.md`'s feature/view list to mention the new
      Statistics page.
- [x] T051 Run `npm run lint`, `npm run format:check`, `node --test web/js/data/statistics.test.js
...
- [x] T052 Update `**Status**` in spec.md from `Draft` to `Implemented` (only once every task above
      is checked off).
