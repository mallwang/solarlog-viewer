...
feature in this codebase to make client-side network requests to a _third-party_ host
(Open-Meteo) rather than only same-origin static files. Principle III's "No Backend
Introduction" restricts _this project_ introducing a server it runs/depends on — it does
...
or users — it _consumes_ an external third-party API (documented in research.md) but
