...
## Dual data source: `web/hist/` vs `web/data/` (added 2026-08-04)

The SolarLog device was replaced on 2026-07-29. The new device cannot append to the old device's
`days_hist.js`/`months.js`/`years.js` — it only overwrites them with its own totals since
installation — so the two eras live in separate directories under `web/`:

- **`web/hist/`** — frozen historical data through 2026-07-28 (`days_hist.js`, `months.js`,
  `years.js`). Never changes again.
- **`web/data/`** — the live device's continuously-overwritten output: `base_vars.js`,
  `min_cur.js`, `days.js` (today's running total), plus its own growing `days_hist.js`/
  `months.js`/`years.js` from 2026-07-29 onward, and daily `min{YYMMDD}.js` files.

`web/js/config.js` defines `INSTALLATION_DATE = '2026-07-29'`. `web/js/data/data-source.js`
resolves which directory a query needs:

- `sourceDirForDate(dateIso)` — for a single day's `min{YYMMDD}.js`, picks `'hist'` or `'data'`
  (a day is never split across both).
- `fetchFromBothSources(filename)` — for `days_hist.js`/`months.js`/`years.js`, fetches both
  `hist/<filename>` and `data/<filename>` in parallel, since a query (e.g. "July 2026" or "year
  2026") can span the installation-date boundary.

`web/js/data/aggregates.js` merges the parsed results:

- `mergeDailyTotals(hist, data)` — concatenates by date (no overlap by construction; the
  data-side wins defensively on a collision) and sorts ascending.
- `mergeMonthlyTotals(hist, data)` / `mergeYearlyTotals(hist, data)` — **sums** `perInverter`
  Wh field-wise for a month/year present on both sides (the installation month/year genuinely has
  production recorded in both the frozen hist snapshot and the new device's running total), and
  passes through months/years present on only one side unchanged.

Every entity below that reads `days_hist.js`/`months.js`/`years.js` now means "the merged result
of both directories," not a single file.
...
Parsed from `hist/months.js` + `data/months.js`, merged via `mergeMonthlyTotals` (see "Dual data
source" above) — one record selected per requested `YYYY-MM`.

| Field            | Type                                  | Source                                                                | Notes                                                                                                         |
| ---------------- | ------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `month`          | `YYYY-MM` string                      | Record date (always the 1st)                                          | —                                                                                                             |
| `perInverter`    | `{ [inverterIndex: number]: number }` | `WRn_yield_Wh`                                                        | Whole-month total per inverter, watt-hours; summed across hist+data if the month spans the installation date. |
| `dailyBreakdown` | `DailyTotal[]`                        | Derived from merged `days_hist.js` (hist+data) filtered to this month | Powers the per-day bars within the monthly chart (FR-010: "daily energy totals per inverter for that month"). |

### `DailyTotal`

Shared shape used by both `MonthlyTotals.dailyBreakdown` and `YearComparisonSeries` below —
parsed from `days.js` (data-only, today's running total) / `days_hist.js` (`hist/days_hist.js` +
`data/days_hist.js`, merged via `mergeDailyTotals`), which share one wire format
...
Parsed from `hist/years.js` + `data/years.js`, merged via `mergeYearlyTotals` — one record per
calendar year.

| Field         | Type                                  | Source                   | Notes                                                                                   |
| ------------- | ------------------------------------- | ------------------------ | --------------------------------------------------------------------------------------- |
| `year`        | number                                | Record date (`01.01.YY`) | —                                                                                       |
| `perInverter` | `{ [inverterIndex: number]: number }` | `WRn_yield_Wh`           | Whole-year total, watt-hours; summed across hist+data for the installation year (2026). |
...
Parsed from `days_hist.js` — `hist/days_hist.js` (the full frozen 2006–2026-07-28 archive) +
`data/days_hist.js`, merged via `mergeDailyTotals` — grouped by year. (Corrected 2026-08-04: an
earlier version of this doc named `daysall.js`/`days_hist_0?.js`; `compare-view.js` has always
read `days_hist.js` directly.)
...
Parsed from `data/min_cur.js` (live-device-only, no historical counterpart) on load and every
5-minute refresh (research.md §7).
